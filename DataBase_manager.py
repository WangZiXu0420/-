...
class DatabaseManager(object):
    ...
    def update(self, data):
        sql = "update %s set name='%s' , title='%s', tel='%s', email='%s', comp='%s', addr='%s' where id=%s" % (
            self.table_name, data['name'], data['title'], data['tel'], data['email'], data['comp'], data['addr'], data['id'])
    ...
    def get_all(self):
        lines = pd.read_sql(self.table_name, self.engine, columns=['name', 'title', 'tel', 'email', 'comp', 'addr', 'id'])
        return lines
...
if __name__ == '__main__':
    ...
    data = {'name': 'name', 'title': 'title', 'tel': 'tel', 'email': 'email', 'comp': 'comp', 'addr': 'addr', 'id': '20201114223505'}
